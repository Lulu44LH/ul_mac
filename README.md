...
**状态机设计**（注：`TRANSMITTING` 瞬态已删除，发送 SR 后保持 `PENDING` 按周期重发，由 `sr_transmitted_flag_` 一次性通知外部）：
```
    ┌─────────┐  数据到达     ┌───────────┐
    │  IDLE   │ ──────────►  │  PENDING  │ ◄── 发送SR后保持PENDING
    └─────────┘              └─────┬─────┘     (仍按周期重发, 经sr_transmitted_flag_通知)
         ▲                         │ SR周期到达且未超限
         │ UL Grant                ▼
         │ 收到              ┌─────────────┐
         ├─────────────────  │  发送 SR     │  (置sr_transmitted_flag_, 仍回PENDING)
         │                   └─────────────┘
         │                          │
         │                     ┌────┴────────┐
         │              成功    │            │ 失败(dsr_transmax)
